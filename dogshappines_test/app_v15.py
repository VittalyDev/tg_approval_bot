...
def serialize_order_v15(c, row, viewer_id, viewer_role):
    ...
    if pet_id:
        pet = c.execute(
            "SELECT photo_path FROM pets WHERE id=?",
            (pet_id,),
        ).fetchone()
        if pet:
...
def executor_slot_reason(c, executor_id, date_str, time_str, duration, exclude_order_id=None):
    ...
    for row in rows:
        if exclude_order_id and int(row["id"]) == int(exclude_order_id):
            continue
        other_start, other_end = v14.order_range(row)
        if other_start and v14.ranges_overlap(start, end, other_start, other_end):
    ...
    for block in blocks:
        try:
            ...
            continue
        if v14.ranges_overlap(start, end, b_start, b_end):
...
def order_edit_payload(c, uid, order, data):
    ...
    pet_id = data.get("pet_id", order["pet_id"])
    try:
        pet_id = int(pet_id)
    except Exception:
    ...
    address = (data.get("address") if "address" in data else order["address"]) or ""
    address = address.strip()
    if item_id != 5 and len(address) < 5:
        raise ValueError("Укажите адрес")
    notes = (data.get("notes") if "notes" in data else order["notes"]) or ""
    notes = notes.strip()[:1200]

    target = order["executor_id"] or order["target_executor_id"]
    if target:
        ep = c.execute(
            "SELECT * FROM executor_profiles WHERE user_id=? AND active=1",
            (target,),
        ).fetchone()
        if not ep:
            raise ValueError("Исполнитель больше недоступен")
        reason = executor_slot_reason(
            c, int(target), date_str, time_str, duration, exclude_order_id=order["id"]
        )
        if reason:
    ...
    schedule_changed = any([
        ...
        time_str != (order["scheduled_time"] or ""),
        int(duration) != int(order["duration_min"] or 60),
    ])
    return {
...
class Handler(v14.Handler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/api/executors/") and path.endswith("/slots"):
            ...
            try:
                duration = max(30, min(2880, int((q.get("duration") or [60])[0])))
            except Exception:
                duration = 60
            try:
                exclude_order_id = int((q.get("exclude_order_id") or [0])[0]) or None
            except Exception:
            ...
            for minute in range(SLOT_START_MIN, SLOT_END_MIN + 1, SLOT_STEP_MIN):
                if minute + duration > 24 * 60:
                    break
                time_str = minute_to_time(minute)
                reason = executor_slot_reason(
                    c, executor_id, date_str, time_str, duration, exclude_order_id
                )
                slots.append({
                    "time": time_str,
    ...
    def do_POST(self):
        ...
        if path.startswith("/api/orders/") and path.endswith("/edit"):
            ...
            if order["status"] not in ("open", "accepted"):
                c.close()
                return self.send_json({"error": "Этот заказ уже нельзя изменить"}, 409)

            try:
                payload = order_edit_payload(c, uid, order, data)
            except ValueError as exc:
                c.close()
                return self.send_json({"error": str(exc)}, 409)

            old_status = order["status"]
            target = payload["target_executor_id"]
            new_status = old_status
            executor_id = order["executor_id"]
            accepted_at = order["accepted_at"]
            event_text = "Клиент изменил параметры заказа"

            if old_status == "accepted" and payload["schedule_changed"]:
                new_status = "open"
                executor_id = None
                accepted_at = None
            ...
            c.execute(
                """UPDATE orders SET
                   ...
                   pet_id=?,pet_name=?,address=?,notes=?,price=?,
                   status=?,executor_id=?,target_executor_id=?,accepted_at=?,updated_at=?
                   WHERE id=?""",
                (
                    payload["scheduled_date"], payload["scheduled_time"], payload["duration_min"],
                    payload["pet_id"], payload["pet_name"], payload["address"], payload["notes"],
                    payload["price"], new_status, executor_id, target, accepted_at,
                    v14.now_iso(), oid,
                ),
            )
            ...
            if target:
                v14.notify_user(
                    target,
                    f"Заказ №{oid} изменён: {payload['scheduled_date']} {payload['scheduled_time']}"
                    + (" · требуется повторное подтверждение" if new_status == "open" else "")
                )
            return self.send_json({
                "ok": True,
                "id": oid,
                "status": new_status,
                "price": payload["price"],
                "requires_reconfirm": old_status == "accepted" and payload["schedule_changed"],
            })
